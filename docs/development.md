...
`high-recall` プロファイルは4ルール（`jp-address-high-recall` /
`person-name-high-recall` / `person-name-structured` / `person-name-romaji`）を含み、
...
（高再現率4ルールを含む）を陽性10件以上まで計算合成ケースで補い、40件のhard negativeと
...
適用します。これによりGCS本文やgenerationを上書きせず、公知PANの再分類と不足した合成正例の
補完を再現できます。migrationは期待spanが明示集合に完全一致する場合だけを対象とし、非公開本文や
ケースIDを公開成果物へ書き出しません。
