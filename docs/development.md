...
1. **source** が走査対象を列挙する。フルスキャンはファイルツリーを walk し、
   ...
   既存行の都合で取りこぼさない。バイナリ判定の前に `decodeUTF16` が UTF-16 の
   BOM（`FF FE`/`FE FF`）を検出し、標準ライブラリの `unicode/utf16` で UTF-8 へ
   変換してから通常のパイプラインに渡す（追加の依存ライブラリなし）。Excel の
   「Unicode テキスト」出力（UTF-16LE）等が対象で、BOM の直後が奇数バイト長・
   不正なサロゲートペアの場合は変換を諦めて通常のバイナリ判定にフォールバックする。
   デコード後の行・列はルーン単位としては正しいが、**元ファイルのバイト
   オフセットとは対応しない**点に注意。`git diff` は UTF-16 ファイルをバイナリ
   扱いするため、この変換はフルスキャン限定で効き、`--staged`/`--diff` の差分
   走査では UTF-16 ファイルはそもそも対象にならない。
2. **detect.ScanLine** が 1 行ごとに処理する。
   ...
   - `.csv`/`.tsv`（[`internal/detect/csv_context.go`](../internal/detect/csv_context.go)、
     フルスキャン限定）は上記のコード文パーサとは別の専用パーサへ分岐する
     （`sourceExtensions` には加えていない。カンマを文区切りとして誤解釈するため）。
     1 行目をヘッダとして RFC 4180 準拠の引用符処理（`""` エスケープ）でフィールドに
     分割し、各列のラベル語をその列の**全データ行**の該当フィールドへ
     `PositiveText`/`NegativeText` として付与する。隣接 ±1 行の source context では
     ヘッダ直下の 1 行しか救えず、3 行目以降のデータ行は文脈を失っていた FN を解消する。
     ヘッダらしくない 1 行目（フィールド数が 2 未満・空フィールドあり・数値主体の
     フィールドあり）は列コンテキストを一切付与しない（安全側 = 現状維持）。フィールド内
     改行で引用符が行末までに閉じないレコードを検出したら、それ以降のレコードへの
     列コンテキスト付与を打ち切る（1 行単位のパーサでは複数物理行にまたがる 1 論理行を
     正しく再構成できず、列がずれた誤帰属を防ぐため）。高再現率モードでは、ヘッダが
     氏名系ラベル語彙（`rule.CSVNameHeaderRe`）と完全一致する列について、各データ行の
     フィールド値を `rule.CSVNameValueRe` で切り出し `rule.ValidCrossLineName`
     （姓名辞書照合）で検証して `person-name-structured` として報告する（フリガナ
     列は辞書が漢字ベースのため対象外）。diff 走査（`sourceLineContextsForDiff`）では
     使わない（hunk はヘッダ行を含まないことが多く、列のずれた誤帰属のリスクが高いため）。
