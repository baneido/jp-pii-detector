...
2. **detect.ScanLine** が 1 行ごとに処理します。
   ...
   - 各ルールのパターンを正規表現でマッチし、`Validate`（チェックディジット等）と
     allowlist で絞り込みます。
     - マッチ前後 `promotionContextWindowRunes`（既定 40 ルーン、`RequireContextWindow` を
       設定したルールはその値）以内にコンテキストキーワードがあれば信頼度を High に
       昇格します。窓を掛けずに行全体を見ると、minified JSON や長い 1 行でキーワードが
       1 個あるだけで行内の全マッチが昇格してしまうためです。
     - `RequireContext` のルールはキーワードがなければ破棄します。こちらの判定は
       `RequireContextWindow` が未設定（0）なら後方互換で行全体を見るため、High 昇格判定の
       フォールバック挙動とは異なります。
