...
（linux/amd64, linux/arm64）イメージを公開しています。タグは `v0.4.3` のような
  ...
  pii-check:
    ...
    steps:
      ...
      - name: Install jp-pii-detect
        run: |
          curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.4.3/scripts/install.sh \
            | JP_PII_DETECT_VERSION=v0.4.3 sh
      - name: Scan
...
pii-check:
  image:
    name: ghcr.io/baneido/jp-pii-detector:v0.4.3
...
jobs:
  pii-check:
    docker:
      - image: ghcr.io/baneido/jp-pii-detector:v0.4.3
    steps:
      - checkout
...
```sh
mise use "github:baneido/jp-pii-detector@v0.4.3"
```
...
```toml
[tools]
"github:baneido/jp-pii-detector" = "v0.4.3"
```
...
```json
{
  "postCreateCommand": "curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.4.3/scripts/install.sh | JP_PII_DETECT_VERSION=v0.4.3 JP_PII_DETECT_INSTALL_DIR=/usr/local/bin sh"
}
```
