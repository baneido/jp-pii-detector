...
（linux/amd64, linux/arm64）イメージを公開しています。タグは `v0.3.6` のような
...
```sh
# カレントディレクトリをフルスキャン
docker run --rm -v "$PWD:/scan" ghcr.io/baneido/jp-pii-detector:v0.3.6

# 引数はそのまま jp-pii-detect に渡る
docker run --rm -v "$PWD:/scan" ghcr.io/baneido/jp-pii-detector:v0.3.6 scan --format json /scan
```
  ...
  pii-check:
    ...
    steps:
      ...
      - name: Install jp-pii-detect
        run: |
          curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.3.6/scripts/install.sh \
            | JP_PII_DETECT_VERSION=v0.3.6 sh
      - name: Scan
...
pii-check:
  image:
    name: ghcr.io/baneido/jp-pii-detector:v0.3.6
...
jobs:
  pii-check:
    docker:
      - image: ghcr.io/baneido/jp-pii-detector:v0.3.6
    steps:
      - checkout
...
```json
{
  "postCreateCommand": "curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.3.6/scripts/install.sh | JP_PII_DETECT_VERSION=v0.3.6 JP_PII_DETECT_INSTALL_DIR=/usr/local/bin sh"
}
```
