...
（linux/amd64, linux/arm64）イメージを公開しています。タグは `v0.4.2` のような
...
docker run --rm -v "$PWD:/scan" ghcr.io/baneido/jp-pii-detector:v0.4.2 scan --format json /scan
```
...
Action の参照とダウンロードするバイナリ版は独立しているため、再現性が必要な場合は
`uses:` のタグと `with.version` を同じ値で明示します。
  ...
  pii-check:
    ...
    steps:
      ...
      - name: Install jp-pii-detect
        run: |
          curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.4.2/scripts/install.sh \
            | JP_PII_DETECT_VERSION=v0.4.2 sh
      - name: Scan
...
走査が不完全な exit 2 では upload ステップへ進めません。部分的な SARIF を
アップロードすると、未走査ファイルの既存アラートが自動解決される恐れがあるため、
ここでは意図的に `if: always()` を付けていません。warning を解消して再実行してください。
...
pii-check:
  image:
    name: ghcr.io/baneido/jp-pii-detector:v0.4.2
...
jobs:
  pii-check:
    docker:
      - image: ghcr.io/baneido/jp-pii-detector:v0.4.2
    steps:
      - checkout
...
[mise](https://mise.jdx.dev) の GitHub バックエンドで GitHub Releases のバイナリを
そのまま管理できます（Go 不要）:

```sh
mise use "github:baneido/jp-pii-detector@v0.4.2"
```
...
```json
{
  "postCreateCommand": "curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.4.2/scripts/install.sh | JP_PII_DETECT_VERSION=v0.4.2 JP_PII_DETECT_INSTALL_DIR=/usr/local/bin sh"
}
```
