...
（linux/amd64, linux/arm64）イメージを公開しています。タグは `v0.1.8` のような
...
docker run --rm -v "$PWD:/scan" ghcr.io/baneido/jp-pii-detector:v0.1.8 scan --format json /scan
```
  ...
  pii-check:
    ...
    steps:
      ...
      - name: Install jp-pii-detect
        run: |
          curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.1.8/scripts/install.sh \
            | JP_PII_DETECT_VERSION=v0.1.8 sh
      - name: Scan
...
pii-check:
  image:
    name: ghcr.io/baneido/jp-pii-detector:v0.1.8
...
jobs:
  pii-check:
    docker:
      - image: ghcr.io/baneido/jp-pii-detector:v0.1.8
    steps:
      - checkout
...
```json
{
  "postCreateCommand": "curl -fsSL https://raw.githubusercontent.com/baneido/jp-pii-detector/v0.1.8/scripts/install.sh | JP_PII_DETECT_VERSION=v0.1.8 JP_PII_DETECT_INSTALL_DIR=/usr/local/bin sh"
}
```
