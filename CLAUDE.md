...
- **`internal/source`** enumerates scan targets: full file-tree walk (parallel) or git-diff (`--staged` / `--diff <range>`). The diff is fetched with `git diff -U3` so each hunk carries surrounding context lines; `scanHunk` → `detect.ScanDiffHunk` scans the hunk so a label on a **logically adjacent** unchanged line can promote a value on an added line, but **only reports findings whose detected value lands on an added line**. Context (unchanged) lines supply positive context *only* — they never drive suppression, so a stale `jp-pii-detector:ignore` or a negative-context unit (`円` etc.) on a context line does not silence a newly-added secret. Skips binaries, >5MB files, and dependency dirs like `node_modules`. Before the binary check, `decodeUTF16` looks for a UTF-16 BOM (`FF FE`/`FE FF`) and transcodes to UTF-8 (stdlib `unicode/utf16`, no new dependency); odd byte length or invalid surrogates fall back to the normal binary check. `internal/source/encoding.go` then additionally transcodes legacy Japanese encodings the BOM check misses, mirroring `decodeUTF16`'s conservative philosophy (fall back to the normal binary/UTF-8 path on any doubt): ISO-2022-JP (detected by `ESC $`/`ESC (` escape sequences in otherwise 7-bit-clean data, decoded via `golang.org/x/text/encoding/japanese.ISO2022JP`), BOM-less UTF-16 (detected by a NUL-byte-ratio heuristic — >30% NUL concentrated at consistently even or odd 2-byte offsets in the first 8KB — then decoded LE/BE with the same `unicode/utf16` logic as `decodeUTF16`), and Shift_JIS/EUC-JP (tried only when the data is not valid UTF-8; a candidate is accepted only if it decodes with no error and no U+FFFD replacement runes and contains at least one hiragana/katakana/kanji rune, preferring the candidate with more Japanese runes and Shift_JIS on a tie). Valid UTF-8 files short-circuit before any of these decode attempts (fast path). Full-scan only — `git diff` treats all of these as binary, so they're never in scope for `--staged`/`--diff`. Decoded positions are rune-accurate for `ScanContent` but don't map back to the original file's byte offsets.
