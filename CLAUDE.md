...
1. **Accuracy must not drift.** `internal/eval` measures precision/recall/F1 against a labeled dataset that lives **outside the repo** (it can contain real-format PII). The `internal/piifixtures` loader reads the dataset and other PII test fixtures from the local JSON path in `$JP_PII_FIXTURES` (fetched from GCS via GitHub OIDC → Workload Identity in CI; see `docs/development.md`). `docs/accuracy.json` (`internal/eval/golden.go`) is the single source of truth for the `low`/badge profile's measured accuracy: `eval_test.go`'s `TestAccuracy` builds a `Golden` from the live `low`-profile results and diffs it against the committed `docs/accuracy.json` with **zero tolerance** (no more per-rule `wantF1` map for that profile). `TestAccuracy` also runs `medium` (CLI-default `min_confidence`) and `high-recall` subtests; `medium` is still gated by a per-rule `wantF1Medium` map (0.005 tolerance) since the golden file only covers the `low` profile, while `high-recall` is measured/logged only. `readme_test.go`'s `TestReadmeBadges` asserts the README badges match, and `dataset_quality_test.go`'s `TestDatasetQuality` guards the dataset itself (unknown rule IDs in `want`/`spans`, exact-duplicate cases, and a ratchet on the count of positive cases missing a `spans` annotation — only an *increase* fails). **Any change to rules or the dataset moves these numbers and breaks CI.** When that happens: run `JP_PII_FIXTURES=<path> go test ./internal/eval -run 'TestGenerateDoc|TestReadmeBadges' -update` to regenerate `docs/accuracy.md`, `docs/accuracy.json`, and the README badges together, then commit all three. CI runs the same command and then `git diff --exit-code docs/accuracy.md docs/accuracy.json README.md`. Without `$JP_PII_FIXTURES`, the eval/badge/accuracy/dataset-quality tests (and all PII-fixture unit tests) `t.Skip`, so `go test ./...` stays green offline.
