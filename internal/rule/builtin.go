...
const (
	...
	// banchi は番地表現（丁目→番→号の連鎖）を最後まで捕捉する終端パターン。
	// 旧実装の `\d{1,4}(?:丁目|番地?|号|(?:-\d{1,4}){1,2})` は最初の単位で
	// 止まり、2丁目10番7号 を「2丁目」までしか拾えなかった。次を捕捉する:
	//   2丁目10番7号 / 2丁目10-7 / 2-10-7 / 10番地の7 / 2丁目（番地なし）
	// 第 1 形は「数字＋単位」で始まり、以降は「(任意の-)数字＋(任意の単位)」を
	// 繰り返す。第 2 形はダッシュ連結の数字列。いずれも末尾は数字または単位で
	// 終わり、裸の数字列（単位もダッシュもない）は対象にしない（RE2 は線形時間
	// なので連鎖の量による破滅的バックトラックは起きない）。
	banchi = `(?:\d{1,4}(?:丁目|番地?の?|号)(?:-?\d{1,4}(?:丁目|番地?の?|号)?)*|` +
		`\d{1,4}(?:-\d{1,4})+(?:号)?)`
)
