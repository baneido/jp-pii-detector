...
func TestRejectYuchoAccountSuffix(t *testing.T) {
	tests := []struct {
		...
		// 再発行区分 1 桁が挟まる形（記号-1桁-番号）の番号部分も棄却する。
		{"12340-2-7654321", 8, false},
		// 再発行区分が 2 桁ならゆうちょ形状ではないため棄却しない。
		{"12340-25-7654321", 9, true},
		// スペース区切り形は、記号の完全な形状（先頭1・末尾0）と番号末尾1を
		// 満たす場合だけ棄却する。
		{"12340 7654321", 6, false},
		{"12345 7654321", 6, true}, // 記号末尾が0でない
		{"12340 7654327", 6, true}, // 番号末尾が1でない
	}
	for _, tt := range tests {
...
func TestValidYuchoAccount(t *testing.T) {
	tests := []struct {
		...
		{"再発行区分1桁を挟む表記", "12390-2-1234561", true},
		{"再発行区分1桁を挟み検査数字が不一致", "12340-2-7654321", false},
...
func TestYuchoValueChecksumStatus(t *testing.T) {
	tests := []struct {
		...
		{"ハイフン形・再発行区分1桁・検査数字成立", "12390-2-1234561", YuchoChecksumValid},
		{"ハイフン形・再発行区分1桁・検査数字が不一致", "10020-3-12345671", YuchoChecksumInvalid},
