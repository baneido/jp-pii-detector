...
// ケースが増えるだけ）。
//
// なお rule.YuchoChecksumInvalid には、公式検査数字を満たさない値だけでなく、
// 記号 5 桁目が 0 以外（総合口座は構造上 5 桁目が 0、振替口座は先頭が 0 のため、
// 「先頭 1 かつ 5 桁目 ≠0」は実在しない形状）も含まれる（rule 側の
// yuchoAccountChecksumOK が形状ゲートで無効化する）。legacy コーパスのゆうちょ
// 陽性にはこの実在しない形状の値も含まれるため、それらもこの移行で陰性へ
// 読み替わる。ロジックは検査数字不成立の場合と共通で、変更はない。
//
// 述語（すべて満たす場合だけ書き換える）:
