...
func TestCrossLineYuchoPairAdjacent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if fs[0].Line != 1 || fs[0].Column != 5 || fs[0].Match != "14040" {
				t.Fatalf("fs[0] = line=%d col=%d match=%q, want 1/5/14040", fs[0].Line, fs[0].Column, fs[0].Match)
			}
...
func TestCrossLineYuchoPairWithPrefixLabel(t *testing.T) {
	d := newDetector(t, "")
	fs := d.ScanContent("f.txt", "ゆうちょ 記号: 14040\n番号: 12345671\n") // jp-pii-detector:ignore
	if len(fs) != 2 {
		t.Fatalf("len(fs) = %d, want 2 (findings=%v)", len(fs), ruleIDs(fs))
	}
	if fs[0].RuleID != "jp-yucho-account" || fs[0].Line != 1 || fs[0].Match != "14040" {
...
func TestCrossLineYuchoPairNegativeCases(t *testing.T) {
	...
	tests := []struct {
		...
		{"順序逆", "番号: 12345671\n記号: 14040\n", nil}, // jp-pii-detector:ignore
		// 記号が "1" 始まりでない。CrossLineYuchoSymbolRe 自体が `1\d{4}` を
		// 要求するためマッチしない。
		{"先頭1でない記号", "記号: 24040\n番号: 12345671\n", nil}, // jp-pii-detector:ignore
		// 番号が "1" 終わりでない。CrossLineYuchoNumberRe 自体が末尾 `1` を
		// 要求するためマッチしない。
		{"末尾1でない番号", "記号: 14040\n番号: 12345670\n", nil}, // jp-pii-detector:ignore
		// 記号・番号とも全桁同一のダミー値。正規表現の形状は満たすが
		...
		// なり、ペアとして成立しない。
		{"値行にignoreマーカー", "記号: 14040\n番号: 12345671 // jp-pii-detector:ignore\n", nil},
		// 「記号: 14040」の次行が「番号: 12345671 円」。CrossLineYuchoNumberRe は
